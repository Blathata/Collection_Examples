...
soup1 = soup('https://parsinger.ru/html/index4_page_1.html')
...
for i in range(1, len(pages)+1):
    soup2 = soup(f'https://parsinger.ru/html/index4_page_{i}.html')
    ...
    for title, descr, price in zip(name, descriptions, rate):
        ...
        "Наименование": title,
        "Бренд": descr[0].split(':')[1].strip(),
        "Форм-фактор": descr[1].split(':')[1].strip(),
        "Ёмкость": descr[2].split(':')[1].strip(),
        "Объем буферной памяти": descr[3].split(':')[1].strip(),
        "Цена": price
        })
